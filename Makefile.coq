...
COQLIBS?=\
  -Q "src" ""\
  -Q "literatecoq" LiterateCoq
COQCHKLIBS?=\
  -R "src" ""\
  -R "literatecoq" LiterateCoq
COQDOCLIBS?=\
  -R "src" ""\
  -R "literatecoq" LiterateCoq
...
uninstall_me.sh: Makefile.coq
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/LiterateCoq && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "LiterateCoq" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/ && rm -f $(NATIVEFILES0) $(GLOBFILES0) $(VFILES0) $(VOFILES0) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
